...
namespace reg_monetario.Datos
{
    public class DatosIngreso
    {
        ...
        public static List<Ingreso> GetAll()//ordenado por fecha
        {
            ...
            SQLiteCommand cmd = new SQLiteCommand("SELECT idIngreso, cantidad, fecha, descripcion, diezmoApartado, idTipo FROM Ingreso ORDER BY fecha;");
            ...
            while (obdr.Read())
            {
                ...
                try
                {
                    i.DiezmoApartado = obdr.GetBoolean(4);
                }
                catch { }
                i.Tipo = DatosTipo.GetById(obdr.GetInt32(5));
                ingresos.Add(i);
        ...
        {
            Conexion.OpenConexion();
            SQLiteCommand cmd = new SQLiteCommand("INSERT INTO Ingreso (cantidad, fecha, descripcion, diezmoApartado, idTipo) VALUES (@cantidad, @fecha, @descripcion, @diezmoApartado, @idTipo);");
        ...
        {
            Conexion.OpenConexion();
            SQLiteCommand cmd = new SQLiteCommand("UPDATE Ingreso SET cantidad = @cantidad, fecha = @fecha, idTipo = @idTipo, descripcion = @descripcion, diezmoApartado = @diezmoApartado WHERE idIngreso = @idIngreso;");
